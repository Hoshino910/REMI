...
REMI 以 **DeepSeek Harness 插件**交付，不需要训练或修改基础模型。实现通过 Harness 的公开 Cordis 扩展点观察会话、写入 SQLite、在模型调用前召回有限记忆，并用自定义 extractive checkpoint 替换较旧的模型可见历史。遗忘曲线、情绪辅助信号和可自定义的透明窗口构成后续技术方向，适用范围包括个人助手、知识问答、持续任务及其他长时交互。

**版本状态：**v0.1 已实现基础存储、时近性评分、预算召回和抽取式压缩；尚未实现情绪模型、访问强化的指数遗忘曲线和完整透明窗口调度。降低实际费用、保持回答质量和实现用户无感连续性，仍需端到端实验验证。
